...
// 去除重复点（基于x坐标）
std::vector<Point> removeDuplicatePoints(const std::vector<Point>& points) {
...
std::vector<Point> smoothFusion(
    ...
    // 1. 寻找重叠区域
    auto [overlap1, overlap2] = findOverlapRegion(wave1, wave2);
    ...
    if (startIdx1 >= wave1.size() || endIdx2 == 0 ||
        ...
        return removeDuplicatePoints(fusedWave);
    }
...
// 连接处平滑处理
void smoothConnectionp(std::vector<Point>& wave, size_t connectionPoint, double smoothingFactor = 0.5) {
...
std::pair<std::vector<Point>, std::vector<Point>> processWaveforms(
    ...
    // 1. 平滑融合两个波形
    std::vector<Point> fusedWave = smoothFusion(wave1, wave2);
    ...
    if (connectionPoint > 0 && connectionPoint < fusedWave.size() - 1) {
        smoothConnectionp(fusedWave, connectionPoint);
    }
    ...
    // 最终去重
    return {
        removeDuplicatePoints(firstPart),
        removeDuplicatePoints(secondPart)
    };
}
